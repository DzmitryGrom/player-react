...
const StatePlayItem = ({ user, duration, title, isSelect, onButtonClick }) => {
  const time = new Date();
  time.setTime(duration);

  return (
    <div onClick={onButtonClick} className={isSelect ? 'playlist-item playlist-item-select' : 'playlist-item'}>
